...
class Price_compare:
    ...
    def __init__(self, master):
        ...
        label.grid(row=8, column=0,padx=(30,10),pady=30)
        ...
        button_find.grid(row=10, column=1, sticky=W, pady=8)
    ...
    def price_amzn(self, key):
        ...
        headers = {
            ...
            "X-Amzn-Trace-Id": "Root=1-65f1a00b-50c6cbc664bc1e27650e5cc7",
        ...
        if self.matches_amzn:
        # If there are matches, proceed as normal
        ...
        # If there are no matches, handle the situation (e.g., by setting a default value)
            self.opt_title.set('Product not available')
            self.var_amzn.set('N/A')
            self.product_link = ''
        '''user_input = self.var.get().title()
        self.matches_amzn = get_close_matches(user_input, list(map.keys()), 20, 0.01)
        self.looktable = {}
